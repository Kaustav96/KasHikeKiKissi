...
export const insertWeddingConfigSchema =
  createInsertSchema(weddingConfig).omit({
    id: true,
    updatedAt: true,
  });
...
export const guests = pgTable(
  "guests",
  {
    ...
    eventsAttending: jsonb("events_attending")
      .$type<string[]>()
      .notNull()
      .default(sql`'[]'::jsonb`),
  ...
  (table) => ({
    slugIdx: index("idx_guests_slug").on(table.inviteSlug),
    rsvpIdx: index("idx_guests_rsvp").on(table.rsvpStatus),
    foodIdx: index("idx_guests_food").on(table.foodPreference),
    sideIdx: index("idx_guests_side").on(table.side),
    createdIdx: index("idx_guests_created").on(table.createdAt),
  })
);
...
export const weddingEvents = pgTable(
  ...
  (table) => ({
    sortIdx: index("idx_events_sort").on(table.sortOrder),
    startIdx: index("idx_events_start").on(table.startTime),
  })
);

export const insertWeddingEventSchema =
  createInsertSchema(weddingEvents).omit({
    id: true,
    createdAt: true,
  });

export type InsertWeddingEvent =
  z.infer<typeof insertWeddingEventSchema>;

export type WeddingEvent =
  typeof weddingEvents.$inferSelect;
...
export const storyMilestones = pgTable(
  ...
  (table) => ({
    sortIdx: index("idx_story_sort").on(table.sortOrder),
  })
);

export const insertStoryMilestoneSchema =
  createInsertSchema(storyMilestones).omit({
    id: true,
    createdAt: true,
  });

export type InsertStoryMilestone =
  z.infer<typeof insertStoryMilestoneSchema>;

export type StoryMilestone =
  typeof storyMilestones.$inferSelect;
...
export const venues = pgTable(
  ...
  (table) => ({
    sortIdx: index("idx_venues_sort").on(table.sortOrder),
  })
);

export const insertVenueSchema =
  createInsertSchema(venues).omit({
    id: true,
    createdAt: true,
  });
...
export const faqs = pgTable(
  ...
  (table) => ({
    sortIdx: index("idx_faq_sort").on(table.sortOrder),
  })
);
...
export const messageLogs = pgTable(
  ...
  (table) => ({
    guestIdx: index("idx_logs_guest").on(table.guestId),
    typeIdx: index("idx_logs_type").on(table.messageType),
    statusIdx: index("idx_logs_status").on(table.status),
    createdIdx: index("idx_logs_created").on(table.createdAt),
  })
);

export const insertMessageLogSchema =
  createInsertSchema(messageLogs).omit({
    id: true,
    createdAt: true,
  });

export type InsertMessageLog =
  z.infer<typeof insertMessageLogSchema>;

export type MessageLog =
  typeof messageLogs.$inferSelect;
