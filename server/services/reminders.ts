 ...
 * Sends personalized reminders to confirmed, opted-in guests via:
 *   - WhatsApp (Meta Cloud API with template messages)
 *   - Telegram (direct bot messages to registered users)
 *
 * Reminder schedule:
 ...
 * Admin receives summary notifications after each reminder batch.
 */
...
async function processReminders(): Promise<void> {
  ...
  try {
    ...
    if (!config.weddingDate) {
      console.log("[Reminders] No wedding date set. Skipping reminders.");
      return;
    }
    ...
    for (const timing of REMINDER_TIMINGS) {
      ...
      // Notify admin of WhatsApp batch results
      if (eligibleGuests.length > 0) {
        await notifyAdminOfReminderBatch(
          timing.label,
          whatsappSuccess,
          whatsappFailed,
      ...
      if (isTelegramAvailable()) {
        ...
        const daysUntil = Math.floor((weddingDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
        const eventName = "Kaustav & Himasree's Wedding";

        const telegramPromises = eligibleGuests.map((guest) =>
          sendTelegramReminder(guest, daysUntil, eventName)
            .then(() => {
              telegramSuccess++;
            })
            .catch((err) => {
        ...
        console.log(`[Reminders] Telegram batch complete: ${telegramSuccess} sent, ${telegramFailed} failed`);

        // Notify admin of Telegram batch results
        if (eligibleGuests.length > 0) {
          await notifyAdminOfReminderBatch(
            timing.label,
            telegramSuccess,
